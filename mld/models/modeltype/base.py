...
from mld.models.metrics import TM2TMetrics, MMMetrics, ControlMetrics, PosMetrics, KeyPosMetrics,ControlMetrics2DProj,KeyPosMetrics2D
    ...
    def configure_metrics(self) -> None:
        for metric in self.metric_list:
            if metric == "TM2TMetrics":
                ...
                self.ControlMetrics2DProj = ControlMetrics2DProj(dist_sync_on_step=self.cfg.METRIC.DIST_SYNC_ON_STEP)
                ...
                self.KeyPosMetrics = KeyPosMetrics(dist_sync_on_step=self.cfg.METRIC.DIST_SYNC_ON_STEP)
            elif metric == 'KeyPosMetrics2D':
                self.KeyPosMetrics2D = KeyPosMetrics2D(dist_sync_on_step=self.cfg.METRIC.DIST_SYNC_ON_STEP)
            else:
