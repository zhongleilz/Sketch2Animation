...
class PoseEncoder(nn.Module):
    ...
    #     参数:
    #     input_mask (torch.Tensor): 输入掩码，形状为 [batch_size, window_len]，
    #                             其中 1 表示有效，0 表示无效。
        
    #     返回:
    #     torch.Tensor: 注意力掩码，形状为 [batch_size, 1, window_len, window_len]，
    #                 用于多头注意力中，True 表示忽略，False 表示考虑。
    #     """
    #     batch_size, window_len = input_mask.shape
    #     # 将输入掩码从 [batch_size, window_len] 转换为 [batch_size, 1, window_len]
    #     expanded_mask = input_mask.unsqueeze(1)
        
    #     # 创建一个与 expanded_mask 形状相同，但值全部为 True 的张量
    #     attn_mask = torch.ones((batch_size, 1, window_len), dtype=torch.bool, device=input_mask.device)
        
    #     # 使用输入掩码更新注意力掩码，input_mask中的1（有效位置）转换为False（不忽略）
    #     attn_mask = attn_mask & (~expanded_mask.bool())
        
    #     # 复制 expanded_mask 沿着第三维，以形成 [batch_size, 1, window_len, window_len]
    #     attn_mask = attn_mask.unsqueeze(-1).repeat(1, 1, 1, window_len)
