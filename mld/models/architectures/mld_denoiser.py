...
class MldDenoiser_PoseAware(nn.Module):
    def __init__(self,
                 latent_dim: list = [1, 256],
                 ff_size: int = 1024,
                 num_layers: int = 6,
        ...
        self.latent_dim = latent_dim[-1]
        self.text_encoded_dim = text_encoded_dim
        self.is_keypose_2d = is_keypose_2d
        ...
        self.pose_encoder = PoseEncoder(use_temporal_atten=False)
        
        if self.is_keypose_2d:
            self.pose_encoder_2d = PoseEncoder(use_temporal_atten=False)

        self.controlnet_cond_embedding_pose = nn.Sequential(
            nn.ReLU(),
            nn.Linear(self.latent_dim, self.text_encoded_dim),
        )

        num_block = num_layers#(num_layers-1)//2
        self.controlnet_down_mid_blocks = nn.ModuleList([
    ...
    def freeze_mld_parameters_wo_pose(self):
        print("Freeze the parameters of MLD")
        for name, param in self.named_parameters():
            if "pose_encoder_2d" not in name:  # Ensure we're checking the parameter name
                param.requires_grad = False  # Freeze parameters

        self.denoiser.eval()  # Set the denoiser to evaluation mode
        self.pose_encoder.eval()
        self.controlnet_cond_embedding_pose.eval()
    ...
    def forward(self,
        ...
        controlnet_residuals_pose = controlnet_residuals
   

        if pose_cond is not None:
            controlnet_residuals_pose = self.cmld_forward(sample,
                timestep,
        ...
        output = self.denoiser(sample,
                timestep,
                encoder_hidden_states,
                timestep_cond=timestep_cond,
                controlnet_residuals=controlnet_residuals_pose,
                )
    ...
    def cmld_forward(self,
                ...
                pose_cond: Optional[torch.Tensor] = None,
                controlnet_residuals: Optional[list[torch.Tensor]] = None,
                pose_mask: Optional[list[torch.Tensor]] = None
                ) -> Union[torch.Tensor, list[torch.Tensor]]:
        ...
        if pose_cond is not None:
            #pose_cond [bsz,4,len,66]
            if self.is_keypose_2d:
                pose_cond = self.pose_encoder_2d(pose_cond,mask=pose_mask.bool())
            else:
                pose_cond = self.pose_encoder(pose_cond,mask=pose_mask.bool())
                

            pose_cond = self.controlnet_cond_embedding_pose(pose_cond)
            # text_emb = text_emb + self.alpha * pose_cond
            text_emb += self.alpha * pose_cond
            # text_emb = self.alpha * pose_cond
