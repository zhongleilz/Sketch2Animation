...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, hidden,text_emb=None, memory=None, mask=None):
        """
        Args:
        ...
        if text_emb is not None:
            batch,lenn,dim = hidden.shape
            text_emb = text_emb.permute(1,0,2)
            text_emb = text_emb.repeat(1,lenn,1)

        q = self.q_linear(hidden)
        k = self.k_linear(text_emb)
        v = self.v_linear(text_emb)
...
class RelMultiHeadedAttention(nn.Module):
    ...
    def forward(self, hidden, pos_emb, memory=None, mask=None,
        ...
        # if torch.isnan(atten_score).any():
        #     print("atten_score contains NaN")

        if mask is not None:
            # apply attention mask
            mask = mask.unsqueeze(1)  # 在第二维增加头维度
            mask = mask.expand(-1, self.n_head, -1, -1)  # 扩展头维度，以匹配头的数量
            # mask = mask.permute(1, 2, 0, 3) 

            atten_score = atten_score.masked_fill(mask, float("-inf"))

        # if torch.isnan(atten_score).any():
        #     print("atten_score after contains NaN")
