...
def create_sketch_materials(sat_factor=1.0, num_materials=7):
    materials = []
    for _ in range(num_materials):
        material = bpy.data.materials.new(name="TransparentSketchMaterial")
        material.use_nodes = True
        material.blend_method = 'BLEND'  # 允许透明度
        nodes = material.node_tree.nodes
        nodes.clear()
        
        # 创建Principled BSDF节点，并设置其为透明
        bsdf_shader = nodes.new(type='ShaderNodeBsdfPrincipled')
        bsdf_shader.inputs['Base Color'].default_value = (1, 1, 1, 1)  
        bsdf_shader.inputs['Alpha'].default_value = 0.0  # 完全透明
        
        # 创建材质输出节点并连接
        material_output = nodes.new(type='ShaderNodeOutputMaterial')
        links = material.node_tree.links
        links.new(bsdf_shader.outputs['BSDF'], material_output.inputs['Surface'])
...
class Joints:
    ...
    def __init__(self,
        ...
        data = rotate_pose(data,angle_x=0,angle_y=-0,angle_z=8) * 0.85
        
        if joint_3d_traj is not None:
            joint_3d_traj = prepare_joints(
                joint_3d_traj,
                canonicalize=canonicalize,
                always_on_floor=always_on_floor,
                jointstype=jointstype,
        ...
        joint_3d_traj = rotate_pose(joint_3d_traj,angle_x=0,angle_y=-0,angle_z=8) * 0.85

        self.data = joint_3d_traj
        self.mode = mode
        ...
        if joint_3d_traj is not None:
            self.trajectory = joint_3d_traj[:, joint_ids[0], [0, 1, 2]]
            if len(joint_ids) > 1:
                self.trajectory_2 = joint_3d_traj[:, joint_ids[1], [0, 1, 2]]
        else:
            self.trajectory = data[:, 0, [0, 1, 2]]
        ...
        self.mat = create_sketch_materials() #JOINTS_MATS
    ...
    def load_in_blender(self, index, mats):
        ...
        head_mat = mats[0]
        body_mat = mats[-1]
        # ['root', 'RH', 'LH', 'BP', 'RK', 'LK', 'BT', 'RMrot', 'LMrot', 'BLN', 'RF', 'LF', 'BMN', 'RSI', 'LSI', 'BUN', 'RS', 'LS', 'RE', 'LE', 'RW', 'LW']
        for lst, mat in zip(self.kinematic_tree, mats):
            for j1, j2 in zip(lst[:-1], lst[1:]):
                ...
                if self.joints[j2] in [
                    ...
                    cylinder_between(skeleton[j1], skeleton[j2], 0.0001, mat)#0.040
                elif self.joints[j2] in [
                        # "LE",
                        # "RE",
                        "LW",
                    ...
                    cylinder_sphere_between(skeleton[j1], skeleton[j2], 0.0001, mat)#0.040
                elif self.joints[j2] in [
                        "LMrot",
                        "RMrot",
                        "RK",
                    ...
                    cylinder_between(skeleton[j1], skeleton[j2], 0.0001, mat)
                elif self.joints[j2] in [
                        "LS",
                        "RS",
                        "LF",
        ...
        mid_joint = (skeleton[self.joints.index("LSI")] + skeleton[self.joints.index("RSI")]) / 2
        mid_joint_low = (skeleton[self.joints.index("LH")] + skeleton[self.joints.index("RH")]) / 2

        new_head = skeleton[self.joints.index("BUN")] + 0.4*(skeleton[self.joints.index("BUN")] - mid_joint)

        cylinder_between(new_head, mid_joint, 0.0001, body_mat)
        cylinder_between(mid_joint, mid_joint_low, 0.0001, body_mat)


        cylinder_between(skeleton[self.joints.index("LSI")], skeleton[self.joints.index("RSI")], 0.0001, body_mat)
        cylinder_between(skeleton[self.joints.index("LH")], skeleton[self.joints.index("RH")], 0.0001, body_mat)

        sphere(0.1, new_head, body_mat)
...
def sphere_between(t1, t2, mat, factor=0.8):
    x1, y1, z1 = t1
    x2, y2, z2 = t2
